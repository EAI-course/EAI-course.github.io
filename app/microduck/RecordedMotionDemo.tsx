...
export default function RecordedMotionDemo() {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      const [THREE,{OrbitControls},{STLLoader},motionResponse,modelResponse]=await Promise.all([
        import('three'),import('three/addons/controls/OrbitControls.js'),import('three/addons/loaders/STLLoader.js'),
        fetch('/microduck/demo-motion.json'),fetch('/microduck/kinematics.json')
      ]);
      if(!motionResponse.ok||!modelResponse.ok) throw new Error('The recorded demonstration could not be loaded.');
      const motion:Motion=await motionResponse.json(); const model=await modelResponse.json();
      ...
      const geoms=model.bodies.flatMap((b:any)=>b.geoms);const loader=new STLLoader();const cache=new Map<string,any>();
      await Promise.all([...new Set<string>(geoms.map((g:any)=>g.mesh))].map(async name=>cache.set(name,await loader.loadAsync('/microduck/meshes/'+encodeURIComponent(name)))));
      if(disposed){cache.forEach(g=>g.dispose());return;}
      const meshes=geoms.map((g:any)=>{const material=new THREE.MeshStandardMaterial({color:new THREE.Color().setRGB(...g.color.slice(0,3)),roughness:.6,metalness:.15});const mesh=new THREE.Mesh(cache.get(g.mesh),material);scene.add(mesh);return mesh;});
      ...
      cleanup=()=>{cancelAnimationFrame(frameId);observer.disconnect();controls.dispose();renderer.dispose();renderer.domElement.remove();cache.forEach(g=>g.dispose());meshes.forEach(m=>m.material.dispose());player.current=null;};
